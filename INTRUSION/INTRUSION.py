...
def regisButton_callback():
    ...
    face_regis_btn = customtkinter.CTkButton(master=main_frame)
    face_regis_btn.pack(pady=10, padx=10)
    face_regis_btn.configure(text="Register Face ID")
    submit_btn = customtkinter.CTkButton(master=main_frame,command=submit_regis_callback)
...
def submit_regis_callback():
    ...
    with open('userList.json', 'w') as f:
        f.write(json.dumps(userData,sort_keys=True, indent=4, separators=(',', ': ')))
